...
public:
  ...
  std::optional<geometry_msgs::PoseWithCovarianceStamped> generateMeasurement(geometry_msgs::PoseWithCovarianceStamped input) {
    ...
    if (view_marker_count == 3) {
      ...
      distance_eigenval_sqrt = 0.1; //(0.1 + randRange(-0.03, 0.03)) * distance;
      ...
      width_eigenval_sqrt  = 0.001; //3 * distance * tan_pixangle;
      height_eigenval_sqrt = 0.001; //3 * distance * tan_pixangle;
    } else if (view_marker_count == 2) {
      /* distance_eigenval_sqrt = (0.5 + randRange(-0.6, 0.6)) * distance; */
      distance_eigenval_sqrt = 0.5; //(0.2 + randRange(-0.07, 0.07)) * distance;
      // distance_eigenval_sqrt = (0.01 + randRange(-0.0003, 0.0003)) * distance;
      /* ROS_INFO_STREAM("[" << ros::this_node::getName().c_str() << "]: distance_eigenval_sqrt = " << distance_eigenval_sqrt); */
      ...
      distance_eigenval_sqrt = 0.5;//(0.01 + randRange(-0.0003, 0.0003)) * distance;
